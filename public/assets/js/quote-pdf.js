...
//             const descParts = [
//                 item.name,  // stays bold via didParseCell
//                 ...bullets.map(x => x) // normal text
//             ];
    ...
    copyBtn.addEventListener('click', async () => {
                ...
                didParseCell: function (data) {
                    ...
                    if (data.section === "body" && data.column.index === 4) {
                        ...
                        if (label === "SUBTOTAL" || label === "VAT 5%" || label === "DISCOUNT") {
                            if (data.row.cells[4]) {
                                data.row.cells[4].styles.fontStyle = "bold";
                                data.row.cells[4].styles.fontSize = 8;
                            }
                            if (data.row.cells[5]) {
                                data.row.cells[5].styles.fontStyle = "bold";
                                data.row.cells[5].styles.fontSize = 8;
                            }
                        }
        ...
        function renderSection(title, linesArray) {
            const safeLines = Array.isArray(linesArray)
                ? linesArray.map(line => String(line || ''))
                : [];
